...
# spec: "" -> random_traffic
# spec: "123" -> 123GB
# spec: "100-500" -> random between 100GB..500GB
pick_total_bytes() {
    local spec="$1"
    if [[ -z "$spec" ]]; then
        random_traffic
    ...
    err "总流量格式错误: $spec (支持: 空=随机 / 数字=GB / min-max=GB范围)"
    exit 1
}
...
modify_config() {
    ...
    read -rp "请输入总上传流量(GB，直接回车保持不变): " new_up_total
    read -rp "请输入总下载流量(GB，直接回车保持不变): " new_down_total
    ...
    [[ -n "$new_up" ]] && yaml_set "$config_file" "network_upload_multiple" "$new_up"
    [[ -n "$new_down" ]] && yaml_set "$config_file" "network_download_multiple" "$new_down"
    [[ -n "$new_up_total" ]] && yaml_set "$config_file" "network_upload_total" "$((new_up_total * 1024 * 1024 * 1024))"
    [[ -n "$new_down_total" ]] && yaml_set "$config_file" "network_download_total" "$((new_down_total * 1024 * 1024 * 1024))"
...
modify_all() {
    ...
    for config in /opt/nezha-fake-*/config.yaml; do
        ...
        [[ -n "$new_up" ]] && yaml_set "$config" "network_upload_multiple" "$new_up"
        [[ -n "$new_down" ]] && yaml_set "$config" "network_download_multiple" "$new_down"
        [[ -n "$new_up_total" ]] && yaml_set "$config" "network_upload_total" "$((new_up_total * 1024 * 1024 * 1024))"
        [[ -n "$new_down_total" ]] && yaml_set "$config" "network_download_total" "$((new_down_total * 1024 * 1024 * 1024))"
